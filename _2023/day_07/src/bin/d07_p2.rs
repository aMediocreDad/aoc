...
impl HandRank for Hand {
    fn rank(mut self) -> (u32, Vec<u8>) {
        ...
        let card_sum = self.iter().map(|card| card.value).collect::<Vec<u8>>();
...
fn is_flush(hand: &Hand) -> bool {
    hand.iter()
        .all(|card| card.value == hand.last().unwrap().value || card.value == 1)
}
...
fn is_full_house(hand: &Hand) -> bool {
    if let Some(first_pair) = get_n_of_a_kind(hand, 3) {
        let remainder = hand
            .iter()
            .cloned()
            .filter(|card| card.value != first_pair && card.value != 1)
            .collect::<Hand>();
